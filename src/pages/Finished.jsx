...
// 終わった取引だけを並べる。
// 車両を引き上げた方（朱色）と、払い終わった方（薄い黄色）を色で分ける。
...
export default function Finished({ onOpen }) {
  ...
  const 引き上げ = shown.filter((r) => r.終了理由 === '引き上げ');
  ...
  return (
    <>
      <div className="bar">
        ...
        {rows && (
          <span className="sub">
            引き上げ {rows.filter((r) => r.終了理由 === '引き上げ').length}名 ／
      ...
      <Note>
        取引が終わった顧客です。<span className="tag t-taken">引き上げ</span> は車両を引き上げた方、
      ...
      <div className="card tw cards">
        <table>
          ...
          <tbody>
            ...
            {[...引き上げ, ...完済].map((r) => (
              <tr key={r.id}
                  className={'clickable ' + (r.終了理由 === '引き上げ' ? 'taken-row' : 'done-row')}
                  onClick={() => onOpen(r.id)}>
                <td className="nm">
                  <b>{r.氏名}</b>
