...
| `public/` | そのまま配るもの。アイコン・`manifest.webmanifest`・`sw.js` |
...
- `/((?!api/|sw\.js|manifest\.webmanifest).*)` → `/index.html`。
  画面はタブ切替だけなのでルーターを持たない。
  受け皿(`sw.js`)と `manifest.webmanifest` は書き換えの対象から外す
  (HTMLが返ると受け皿が入らなくなる)
- `/sw.js` は `no-store`。新しい受け皿を必ず取りに行かせる
...
スマホ・iPad のホーム画面に置くと、アドレス欄の無い1枚の画面として開く。

| | |
|---|---|
| `public/manifest.webmanifest` | 名前・アイコン・`display: standalone` |
| `public/sw.js` | 受け皿。**`/api` はためない**（顧客の情報を端末に残さない） |
| `public/icon-*.png` | 192 / 512 / 切り抜き用512 / iOS用180 |

アイコンは `tools/icons.mjs` で描いて撮っている（外部の画像を持ち込まない）。
作り直すときは `node tools/icons.mjs`。

**置きかた。**

- iPhone / iPad … Safari で開き、共有ボタン →「ホーム画面に追加」
- Android … Chrome のメニュー →「アプリをインストール」
- パソコン … Chrome / Edge のアドレス欄の右に出るインストールの印

**受け皿の決めごと**(`sw.js` の頭にも書いてある)

1. `/api` は**ためない**。古い数字を出すくらいなら「つながりません」と言う
2. 画面の入れ物(HTML)は毎回ネットを見る。取れなければ、ためた分を出す
3. `/assets/…` は名前に中身の指紋が入っているので、ためて使う
4. 新しい版が来たら待たずに入れ替え、画面を読み直す
   (デプロイのあとに古い画面が残り続けるのを防ぐ)

**電波が無いときは、開くところまでしかできない。** 台帳の中身はサーバーにあるため。

## スマホ・iPad の見え方

- **幅760px以下**(スマホ)… 表を横スクロールさせず、1件ずつの札にする。
  項目名は各セルの `data-label` から出す
- **761〜1180px**(iPad)… 表のまま。顧客ページは980px以下で縦に積む
- iPhone の切り欠きは `env(safe-area-inset-*)` で避ける
- 指で押す端末では、ボタンの高さを44px以上にする
- 入力欄は16px以上(iOS が勝手に拡大するのを防ぐ)
