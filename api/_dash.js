...
export async function ダッシュボード(sql) {
  ...
  // **1人につき1行にする。**同じ人が複数の月で切れていても、電話するのは一度。
  // 何度も名前が並ぶと、その人だけで一覧が埋まって他の人が埋もれる。
  const 切れ生 = [];
  for (const m of 月) {
    for (const r of m.行) {
      if (r.後回し && r.後回し.切れ) {
  ...
  // いちばん長く過ぎているものを、その人の代表にする
  切れ生.sort((a, b) => b.過ぎた日数 - a.過ぎた日数);
  const 見た顧客 = new Set();
  const 約束切れ = 切れ生.filter((x) => {
    if (見た顧客.has(x.顧客id)) return false;
    見た顧客.add(x.顧客id);
    return true;
  });
