...
// 決めごと（トリガや関数）が古いままのしるし。
//
// 作り直しは「テーブルや列が無い」ときにしか走らない。
// ところが決めごとを書き換えただけの回は、列も表も増えないので
// きっかけが無く、本番にいつまでも届かない。
// 実際に「入金の無い顧客なら記録も消せる」に変えた回で、これが起きた。
//
// 追記のみに弾かれたときも、いちどだけ作り直してからやり直す。
// 台帳の作りとして正しい操作なら、これで通る。
// 本当に許されない操作なら、やり直しても同じところで弾かれる。
const 古い決めごと = (e) => /記録は追記のみです/.test((e && e.message) || '');

async function withSetup(run, t, p) {
  try {
    return await run(t, p);
  } catch (e) {
    if (!足りない.has(e && e.code) && !古い決めごと(e)) throw e;
    if (!作り直し) {
      console.warn('[db] 作り直します:', e.message);
