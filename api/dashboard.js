...
// ・車を回収した方と、動作を試すための顧客は外す。経営者が見る数字なので混ぜない
...
export default async (req, res) => {
  ...
  try {
    ...
    const [顧客, 予定, 充当, 約束, トラブル] = await Promise.all([
      // 車を回収した方は督促しない。試すための顧客は数字に混ぜない
      ...
      sql(`SELECT tr.id, tr.customer_id, tr.kind, tr.memo, ${日時('tr.created_at')} AS at,
                  c.name, c.monthly_amount
             FROM trouble tr
             JOIN customer c ON c.id = tr.customer_id
            WHERE tr.resolved = false AND c.archived = false AND c.is_test = false
            ORDER BY tr.id DESC`),
    ]);
    ...
    for (const p of 約束) {
      ...
      if (!行.後回し) 行.後回し = { 件数: 0, 日: null, 時刻: null, 金額: 0 };
      行.後回し.件数++;
    ...
    return ok(res, {
      ...
      月,
      トラブル: トラブル.map((x) => ({
        id: x.id, 顧客id: x.customer_id, 氏名: x.name,
        料金: x.monthly_amount, 種類: x.kind, 内容: x.memo || '', 日時: x.at,
      })),
    });
  } catch (e) {
