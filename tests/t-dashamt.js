...
(async () => {
  ...
  if (早い日 < 遅い日) {
    ...
    check('予定回収額は月まるごとの80,000円', 今 && 今.予定回収額 === 80000,
      今 && 今.予定回収額);
    ...
    check('予定＝期日前＋未回収＋回収済み',
      今 && 今.予定回収額 === 今.期日前額 + 今.未回収額, [今.予定回収額, 今.期日前額, 今.未回収額]);
    ...
    // 消えなければ予定回収額は変わらない
    if (今2) {
      check('予定回収額は80,000円のまま', 今2.予定回収額 === 80000, 今2.予定回収額);
